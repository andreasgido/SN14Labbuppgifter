...
--		DateOfReturn AS DATEADD(DAY, 14, DateOfLoan),
		DateOfReturn datetime NOT NULL
			CONSTRAINT DF_DateOfReturn DEFAULT DATEADD(DAY, 14, GETDATE()),
...
IF NOT EXISTS (SELECT * FROM sys.objects 
WHERE object_id = OBJECT_ID(N'dbo.vBookOnLoan'))
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.objects 
WHERE object_id = OBJECT_ID(N'dbo.vAvailableBooks'))
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.objects
WHERE object_id = OBJECT_ID(N'dbo.uspMakeLoanOfBook'))
BEGIN
	EXEC ('CREATE PROCEDURE dbo.uspMakeLoanOfBook
	(
		@CopyOfBookID int,
		@CustomerID int	
	)
	...
	BEGIN
		SET NOCOUNT ON;
		INSERT INTO dbo.LoanOfBook(CopyOfBookID, CustomerID)
		VALUES (@CopyOfBookID, @CustomerID)

		UPDATE dbo.CopyOfBook
		SET StatusID = 2
		FROM dbo.CopyOfBook AS COB
			INNER JOIN LoanOfBook AS LOB ON COB.CopyOfBookID = LOB.CopyOfBookID
		WHERE COB.CopyOfBookID = @CopyOfBookID 
	END ')
END	
GO

-- Lånar ut boken med KopiaID 9 till kund med kundnummer 4
EXEC dbo.uspMakeLoanOfBook @CopyOfBookID = 9, @CustomerID = 4
GO
--SELECT * FROM LoanOfBook
--GO
...
BEGIN
	...
	BEGIN
		...
		UPDATE dbo.LoanOfBook
		SET	LoanCompleted = 1
			,	DateOfReturn = GETDATE()
		FROM dbo.LoanOfBook
		WHERE CopyOfBookID = @CopyOfBookID
...
EXEC dbo.uspReturnBook @CopyOfBookID = 9
GO
